...
tags:
  ...
  - エラーレート
private: false
updated_at: '2025-05-18T00:58:05+09:00'
...
スロークエリも試してみましょう。
自作アプリケーションの[ユーザー一覧（スロークエリ）]ボタンの内部処理は下記のとおり、10秒スリープ後にPostgreSQLのユーザー一覧を返却しています。
...
スロークエリのアラート設定は前述通り、実行時間が5秒を超えるクエリがある状態が1分間継続することが条件であるため、[ユーザー一覧（スロークエリ）]ボタンを1分間の間（計6回）押し続けてみましょう。
...
本記事では、Webサービスの品質劣化を未然に防ぐために、PrometheusとGrafanaを活用してスロークエリやHTTPエラーを監視する方法を解説しました。
